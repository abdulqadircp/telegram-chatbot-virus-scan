...
def create_graph(chat_id,message_id,user_id):
    ...
    bot.send_photo(chat_id=chat_id, photo=open("request_types_distribution.png", 'rb'),reply_to_message_id=message_id)
    #cursor.close()
...
def send_file_to_group(file_path,message,message_id):
  with open(file_path, 'rb') as file:
    bot.send_document(message.chat.id,file,reply_to_message_id=message_id)
...
def handle_files(message):
  if message.caption:
        if message.caption.startswith('/scan_file'):
            ...
            insert_data_to_RequestLog(user_id=message.from_user.id,RequestType="File")
            ...
            send_file_to_group(file_path=file_path,message=message,message_id= message.message_id)
...
def handle_urls_IP(message):
    if "scan_url :" in  message.text:
            ...
            insert_data_to_RequestLog(user_id=message.from_user.id,RequestType="URL")
            ...
            insert_data_to_RequestLog(user_id=message.from_user.id,RequestType="IP")
            ...
            create_graph(message.chat.id,message.message_id,user_id=message.from_user.id)
